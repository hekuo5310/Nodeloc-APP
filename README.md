...
- **发帖设备信息**：对齐官方 APP 的 `mobile_source` 功能 —— 移动端发帖 / 回复时在帖子下方显示设备徽章（如「Pixel 8 Pro」），支持 关闭 / 仅平台 / 品牌 / 完整型号 四档设置；Android 按官方逻辑识别子品牌（Redmi / POCO / iQOO / realme / HONOR / Nothing），iOS 输出友好型号名；桌面端不发送
